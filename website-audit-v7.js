  ...
  function buildUI() {
    ...
    panel.innerHTML = [
      ...
      '<div id="wa-score-bar">',
        ...
        '</div>',
        '<button id="wa-run-btn" type="button">Run Audit</button>',
      '</div>',
      '<div id="wa-body">',
...
document.getElementById('wa-body').addEventListener('click', function (e) {
...
if (runBtn) {
  ...
  function finishRun() {
    waIsRunning = false;
    var btn = document.getElementById('wa-run-btn');
    if (btn) {
      btn.disabled = false;
  ...
  data.append('action', cfg.action || 'wa_fetch_url');
